...
1. Для subpath `/tg` и `/transcription`: какие именно компоненты должны жить под subpath (только API, или полноценный UI), и какой допустим объём переписывания (минимальный патч vs перенос UI в KB-RING)?
2. Формат web-URI через портал для TG: какой шаблон URL будем считать каноническим (например `/tg/chat/<id>/msg/<id>`, `/tg/digest/<id>`), и какие страницы мы обязаны сделать кликабельными на шаге 1?
3. Добавление “ещё нескольких чатов” при миграции TG: какие чаты (ID/username) и где источник правды (channels.json)? Нужен ли отдельный режим “dry-run” перед включением записи в новую БД?
4. Мониторинг “типа Zabbix”: у нас уже есть Zabbix server куда слать метрики/агента, или ставим локально Zabbix agent + существующую систему мониторинга (например netdata) и только настраиваем алерты?
