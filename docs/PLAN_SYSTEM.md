...
2. OCR: используем cloud OCR ключи из TG стека, обработка последовательная (один объект за раз), с таймаутами и устойчивостью к “долго/не отвечает”.
3. Форматы MVP для вложений: `pdf (включая сканы)`, `jpg/png`, `docx`, `txt/md/html`.
4. Enforcement лимита ~30% ресурсов: docker limits для KB-RING worker/api + ограничение параллелизма для embeddings/rerank/OCR.
