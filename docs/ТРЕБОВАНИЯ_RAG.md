...
- `rag-tech`: строгий инженерный отчёт в фиксированном формате, retrieval+rerank, citations обязательны. При отсутствии `OPENAI_API_KEY` допускается локальный fallback на Ollama (черновик).
