...
## 9) Открытые вопросы (минимум на запуск)

1. Список новых чатов для добавления при миграции TG (id/username) и где фиксируем конечный список.
2. Реальные владельцы аккаунтов/ключей AssemblyAI и OpenAI для финальной ротации в проде (технически ротация включена в шаг 1, но нужны владельцы).
3. Политика переноса media на шаг 1: полный перенос или выборочный (влияет на срок и время индексирования).
