...
2. OCR для “всех вложений” на шаге 1:
   - какой OCR движок используем (существующий из TG стека, cloud API, tesseract локально)?
   - какие форматы считаем обязательными (pdf-scans/images/docx) и какие можно отложить?
3. Лимит ресурсов: как именно enforce “не более 30% ресурсов сервера”:
   - docker limits (cpu/mem) для worker/api?
   - ограничение параллелизма в воркере (embeddings/rerank/OCR)?
