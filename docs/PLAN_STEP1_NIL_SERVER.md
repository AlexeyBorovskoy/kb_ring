...
1. Subpath для TG: какой конкретный стек/роутинг у TG UI (FastAPI templates, SPA, другое) и где точка конфигурации base_url/root_path? (нужно, чтобы патч был минимальным и воспроизводимым).
2. Subpath для transcription: есть ли требование сохранять прямые ссылки на статику/результаты без переписывания фронта, или допускаются изменения в `static/index.html` под base href?
3. Импорт TG:
   - предпочтительный формат дампа (pg_dump custom/plain)?
   - какие именно таблицы/объекты считаем “всё TG” (сообщения, дайджесты, пользователи, настройки, аудит, вложения, очереди)?
4. Secrets/ротация:
   - куда именно на сервере кладём единый `secrets.env` (путь) и кто владелец (root/alexey)?
   - есть ли процесс ротации ключей AssemblyAI/OpenAI (кто владелец аккаунтов), и где фиксируем факт ротации (logbook)?
5. Мониторинг:
   - есть ли уже Zabbix server (адрес) и схема регистрации хоста, или выбираем netdata/Prometheus для шага 1?
