...
- **Worker** (`kb_ring/worker/`): фоновая обработка `op.jobs` (пока: чанклинг + FTS).
...
- `tac.embeddings`: pgvector (каркас; пайплайн эмбеддингов добавим отдельно).
...
- `OPENAI_API_KEY`: ключ OpenAI (нужен только для `mode=rag`).
...
- `GET /api/v1/search` делает FTS по `tac.chunks.tsv` и ранжирует по `ts_rank`.

Планируемое расширение:
- эмбеддинги в `tac.embeddings` (pgvector);
- гибридный скоринг: FTS rank + vector similarity.

## Как устроен RAG (этап 1)

В `POST /api/v1/chat/sessions/{id}/message` при `mode=rag`:
1. Выполняется retrieval (FTS, top-K).
...
- Сейчас индексация делает только FTS (чанки + tsvector). Векторный поиск и эмбеддинги будут добавлены следующим шагом.
- Для работы `mode=rag` требуется `OPENAI_API_KEY` в `kb_ring/docker/.env` (этот файл в `.gitignore`).
